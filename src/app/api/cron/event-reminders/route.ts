...
import { sendEventReminder, type EventSignupEmailPayload } from "@/lib/email";
...
export async function GET(req: NextRequest) {
  ...
  return logCronRun("event-reminders", "0 15 * * *", async () => {
    ...
    const dueEvents = events.filter(
      (e) => e.groupSignup && window.has(e.dateISO) && !e.isPast,
    );
    ...
    for (const ev of dueEvents) {
      const { data: rows, error } = await sb
        .from("event_signups")
        .select(
          "id, lead_name, lead_email, party_size, stay_type, nights_from, nights_to",
        )
      ...
      for (const r of (rows ?? []) as Partial<EventSignupRow>[]) {
        if (!r.id || !r.lead_email || !r.lead_name || !r.stay_type) continue;
